...
export default function RegisterPage() {
  ...
  const onSubmit = async (data) => {
    ...
    try {
      const result = await signup(data.name, data.email, data.password, data.domain);
      if (result.success) {
  ...
  const handleGoogleSignup = async () => {
    ...
    try {
      ...
      if (error.code === 'auth/popup-closed-by-user' || error.code === 'auth/cancelled-popup-request') {
        ...
        setStatus({ type: 'error', message: error.message || 'Google Sign Up failed. Please try again.' });
      }
      setLoading(false);
    }
  };
        ...
        >
          ...
          >
            ...
            <Stack sx={{ gap: 1, mb: 3 }}>
              ...
              <Typography variant="body2" sx={{ color: 'text.secondary' }}>
                Join us to build a responsive portfolio site.
              </Typography>
            </Stack>
            ...
            <AnimatePresence mode="wait">
              {status.message && (
                ...
                >
                  <Alert severity={status.type === 'info' ? 'info' : status.type === 'success' ? 'success' : 'error'} sx={{ mb: 2.5, borderRadius: 3 }}>
                    {status.message}
                  </Alert>
                </motion.div>
              )}
            ...
            <form onSubmit={handleSubmit(onSubmit)}>
              <Stack sx={{ gap: 2 }}>
                ...
                <Stack>
                  <FieldLabel name="Primary Focus Area" />
                  <Controller
                    control={control}
                    name="domain"
                    render={({ field }) => (
                      <Select
                        ...
                        sx={{ borderRadius: 3, bgcolor: 'background.default' }}
                        startAdornment={
                          <InputAdornment position="start" sx={{ pl: 0.5, pr: 0.75, color: 'text.secondary' }}>
                            <SvgIcon name="tabler-code" size={18} color="inherit" />
                          </InputAdornment>
                        }
                      >
                        ...
                        <MenuItem value="other">Other / General Practitioner</MenuItem>
                      </Select>
                    )}
                  />
                </Stack>
                ...
                <Stack>
                  <FieldLabel name="Confirm Password" />
                  <OutlinedInput
                    ...
                    startAdornment={
                      <InputAdornment position="start" sx={{ pl: 0.5, color: 'text.secondary' }}>
                        <SvgIcon name="tabler-lock-check" size={18} color="inherit" />
                      </InputAdornment>
                    }
                  />
                  ...
                  <FormControlLabel
                    ...
                    label={
                      <Typography variant="body2" sx={{ color: 'text.secondary', userSelect: 'none' }}>
                        I agree to the{' '}
                        <Link component={NextLink} href="/terms-condition" target="_blank" sx={{ color: 'primary.main', textDecoration: 'none', fontWeight: 600 }}>
                          Terms of Service
                        </Link>{' '}
                        and{' '}
                        <Link component={NextLink} href="/privacy-policy" target="_blank" sx={{ color: 'primary.main', textDecoration: 'none', fontWeight: 600 }}>
                          Privacy Policy
                        </Link>
                      </Typography>
                    }
                    sx={{ ml: -0.5 }}
                  />
                  {errors.terms?.message && <ErrorMessage message={errors.terms?.message} />}
                </Stack>
